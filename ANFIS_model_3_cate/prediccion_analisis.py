...
from Training_ANFIS import train_anfis_multiclass,predict_multiclass
from anfis_sugeno import compute_weights, n_vars, n_classes, reglas
...
# Añade estas funciones al archivo

def predict_sugeno_multiclass(X, mf_params, theta):
    """
    ...
    for x in X:
        probs = predict_multiclass(x, mf_params, theta)
        y_probs.append(probs)
        y_pred.append(np.argmax(probs))
    
    return np.array(y_pred), np.array(y_probs)

def predict_con_explicacion_multiclass(x_sample, mf_params, theta, n_top_reglas=5):
    """
    Predice una muestra individual y explica qué reglas fueron más activas (multiclase)
    """
    w_bar = compute_weights(x_sample, mf_params)
    
    # Calcular contribuciones por clase
    contribuciones_por_clase = []
    ...
    for c in range(n_classes):
        ...
        contribuciones_por_clase.append(contribuciones)
    
    # Calcular probabilidades finales
    probs = predict_multiclass(x_sample, mf_params, theta)
    prediction = np.argmax(probs)
    
    return {
        'prediccion_continua': probs,
        'prediccion_binaria': prediction,
        'contribuciones_por_clase': contribuciones_por_clase,
        'top_reglas_activas': [contribuciones[:n_top_reglas] for contribuciones in contribuciones_por_clase]
    }

# Actualizar la función generar_reporte_completo para multiclase
def generar_reporte_completo_multiclass(X_train, y_train, mf_opt, theta_opt, save_plots=True):
    """
    Genera reporte completo con evaluación del modelo multiclase
    """
    print("="*60)
    print("REPORTE COMPLETO - ANFIS MULTICLASE PARA DETECCIÓN DE TUMORES CEREBRALES")
    print("="*60)
    
    # 1. Predicciones del modelo
    y_pred, y_probs = predict_sugeno_multiclass(X_train, mf_opt, theta_opt)
    ...
    print(classification_report(y_train, y_pred, 
                              target_names=['Meningioma', 'No Tumor', 'Pituitaria'],
                              digits=4))
    ...
    if save_plots:
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                   xticklabels=['Meningioma', 'No Tumor', 'Pituitaria'],
                   yticklabels=['Meningioma', 'No Tumor', 'Pituitaria'])
        plt.title('Matriz de Confusión - Multiclase')
        plt.ylabel('Valores Reales')
        plt.xlabel('Predicciones')
        plt.tight_layout()
        plt.savefig('matriz_confusion_multiclase.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    for class_id in range(n_classes):
        ...
        # Obtener modelo para esta clase
        start_idx = class_id * len(reglas) * (n_vars+1)
        end_idx = (class_id+1) * len(reglas) * (n_vars+1)
        theta_class = theta_opt[start_idx:end_idx]
        
        # Analizar reglas para esta clase
        analizador = AnalizadorReglasANFIS(mf_opt, theta_class, X_class, y_class)
        top_reglas = analizador.obtener_top_reglas(5)
        
        resultados_por_clase[class_id] = {
            'nombre': ['Meningioma', 'No Tumor', 'Pituitaria'][class_id],
            'muestras': len(X_class),
            'top_reglas': top_reglas
        }
    
    return {
        'metricas': classification_report(y_train, y_pred, output_dict=True),
        'resultados_por_clase': resultados_por_clase
    }
...
def ejecutar_analisis_completo_mejorado():
    ...
    mf_opt, theta_opt = train_anfis_multiclass(X_train, y_train, swarmsize=30, maxiter=15)
